...
def histogram_predict(belief, left_encoder_ticks, right_encoder_ticks, grid_spec, robot_spec, cov_mask):
    ...
    left_distance = left_encoder_ticks * 2 * math.pi * robot_spec['wheel_radius'] / robot_spec['encoder_resolution']
    right_distance = right_encoder_ticks * 2 * math.pi * robot_spec['wheel_radius'] / robot_spec['encoder_resolution']

    # Calculate linear and angular velocity
    v = (left_distance + right_distance) / 2  # linear velocity
    w = (right_distance - left_distance) / robot_spec['wheel_baseline']  # angular velocity
    # DONE

    # THE FOLLOWING LINES SEEM TO MAKE NO SENSE TO ME HERE:
    ...
    for i in range(belief.shape[0]):
        for j in range(belief.shape[1]):
            ...
            if belief[i, j] > 0:
                ...
                i_new = int(round((d_t[i, j] - grid_spec['d_min']) / grid_spec['delta_d']))
                j_new = int(round((phi_t[i, j] - grid_spec['phi_min']) / grid_spec['delta_phi']))
                i_new = max(0, min(i_new, belief.shape[0] - 1))
                j_new = max(0, min(j_new, belief.shape[1] - 1))
                # DONE
...
def histogram_update(belief, segments, road_spec, grid_spec):
    ...
    if measurement_likelihood is not None:
        ...
        if np.any(np.isnan(belief)) or np.any(np.isinf(belief)):
            print("Warning: NaN or infinite values detected in belief. Resetting to uniform distribution.")
            belief[:] = 1.0 / belief.size
        else:
            total = np.sum(belief)
            if total > 0:
                belief /= total
            else:
                print("Warning: Belief sum is zero. Resetting to uniform distribution.")
                belief[:] = 1.0 / belief.size
        # DONE
